...
export default function AttendanceReview() {
  ...
  const saveEdit = async (id) => {
    ...
    try {
      await apiClient.put(`/admin/attendance/${id}`, editForm);
  ...
  const approve = async (id) => {
    try {
      await apiClient.post(`/admin/attendance/${id}/approve`);
  ...
  const submitReject = async (id) => {
    ...
    try {
      await apiClient.post(`/admin/attendance/${id}/reject`, { reason: rejectReason });
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        ...
        {['hod_approved', 'pending', 'hod_rejected', 'approved', 'rejected', ''].map((s) => (
          <button
            key={s || 'all'}
              ...
              <tr key={r.id} className="border-t border-gray-100 align-top">
                ...
                <td className="px-4 py-2">
                  ...
                  {r.status === 'rejected' && r.rejection_reason && (
                    <div className="text-xs text-gray-400 mt-1 max-w-[160px]">{r.rejection_reason}</div>
                  )}
                  {r.status === 'hod_rejected' && r.hod_rejection_reason && (
                    <div className="text-xs text-gray-400 mt-1 max-w-[160px]">HoD: {r.hod_rejection_reason}</div>
                  )}
                </td>

                <td className="px-4 py-2">
                  <div className="flex justify-end gap-2">
                    {r.status === 'hod_approved' && editingId !== r.id && (
                    ...
                    {r.status === 'approved' && (
                      <button title="Download PDF" onClick={() => handleDownload(r)} className="text-gray-500 hover:text-blue-600">
